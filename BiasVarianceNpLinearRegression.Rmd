...
for (p in 2:9) {  # Only go up to p=9 because intercept will be 10th.
  out <- replicate(n=num_samples, c(p, sample_train_test(X, y, num_rows, p)))
  df <- rbind(df, t(out))
}
...
Another idea to see the overparameterized regime is to lower the number of rows.  Then we hit the overparameterized regime for lower p.  For example, if num_rows is 5, then interpolation is achieved at p=5, and after that it is overparametrized.
