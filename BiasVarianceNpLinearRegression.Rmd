...
Let's crank up the number of available parameters by using every combination of pairs of columns.
...
ggplot(df_mean, aes(p, mean_error, color=name), group_by(name)) + geom_line() + geom_point() +
  coord_cartesian(ylim = c(0, 300)) 
```
Finally this is enough parameters to see the double descent clearly, although I cannot explain why it happens at exactly 26 parameters.  There is a period of sustained low test error, and while it is noisy, the variance is decreasing as we add more parameters.  I bet if I increased the sample size I could get a clearer trend.

```{r}
num_rows <- 10 # Back to 10
num_samples <- 100 # Crank this up to reduce noise!
df <- data.frame()
for (p in 2:91) { # 9 original + 9 * 9 + intercept
  out <- replicate(n=num_samples, sample_train_test(X, y, p, num_rows))
  df_local <- cbind(rep(p, num_rows), t(out))
